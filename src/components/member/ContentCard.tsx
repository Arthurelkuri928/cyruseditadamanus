...
const ContentCard = ({ id, title, year, rating, image, status = "on" }: ContentCardProps) => {
  ...
  return (
    <div 
      className="relative cursor-pointer group transition-all duration-300 hover:transform hover:scale-105"
      onClick={handleCardClick}
    >
      <img 
        src={image} 
        alt={title} 
        className="w-full aspect-[2/3] object-cover rounded-sm shadow-lg" 
      />
